...
| `atelier/benchmarks/m3/` | M3 三臂对照实验台（P0-3）：protocol.md（noskill/skill/react × 首遍正确率）+ 6 任务书（task1-3 冒烟正控层 + task4-6 加难层：流式 keyed each / 跨组件事务 / token 纪律）+ grade.mjs 评分器（acceptance harness，正控参考解在 reference/）+ report.mjs §7 出数。改评分器后必跑六正控回归。 |
...
| `atelier/mcp/` | stdio MCP Server（24 工具单源生成，live 工具需一个运行中的应用 dev 面）。 |
