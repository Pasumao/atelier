...
## 5. 出数前置：负控集与评分独立性（2026-09-06 锐评增补，未过不得出数）

- **负控 fixture 集**（已落地，机检 = `node atelier/benchmarks/m3/negative-check.mjs`，exit 0 才许出数）：
  `harness/fixtures/negative/` 下每枚 = 对参考解做单一变异的"差一点错"样本（task4 乱序顺序破坏/
  错误徽标缺失、task5 子组件私用 $state/计数过期、task6 config 缺 token/引用未登记 token 走 ATR-204），
  评分器必须全数判 FAIL——正控六绿只证明"对的能给过"，负控才证明"错的抓得住"。新增变异样本
  = 丢进该目录（命名 `<taskId>-<mutation>`），脚本自动纳入。
- **react 臂评分独立性**：rubric 评分者 ≠ 实验编排者；或双人独立盲评、分差 >2 分时第三人仲裁。
  理由：atelier 两臂是机械评分，react 臂主观评分若出自同一作者，臂间不对称即系统性偏置。
- **样本量措辞纪律**：每格 5 个二值 run 的置信区间宽于 +15pt 判据——出数报告必须附区间/样本量
  限定语，禁止把小样本差异写成定论（写入 §7 判定输出）。
- **外部第三方执行**：三臂 run 由非编排者的独立会话执行（每臂全新会话本就是协议铁律；编排者
  不亲自担任任何一臂的代理会话）。
