...
- **负控 fixture 集**：task4-6 各造 ≥2 个"差一点错"变异样本（如：each 无 key、解析丢行、徽标漏分支、
  子组件私用 $state、引用未登记 token），**断言评分器必须红**。正控六绿只证明"对的东西能给过"，
  负控才证明"错的东西抓得住"——判别力的两半。落点建议 `harness/fixtures/negative/<task>/`。
