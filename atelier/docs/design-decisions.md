...
- **组件组合语义修订（2026-09-06，F-5/P3-6，锐评整改）**：动态属性从「挂载期一次性求值」升级为
  **响应式 props**——实现为 prop 信号 + getter（子组件 `props.title` 语法不变，读属性即读信号即被
  追踪），父侧 effect 求值属性表达式回写信号；解释器与 codegen 双路径同源（`rt.bindProp`）。
  同时落地 **effect 所有权**（分支/行级析构栈）：`{#if}` 换支、`{#each}` 行移除/全清重建时，旧分支
  创建的 effects 与嵌套组件实例随之 dispose（与 HMR 实例级 `__effectSink` 正交的两级回收）。
  红检先红后绿（`tests/f5-kernel.test.ts` 红检①②+加固③④）；诚实边界：组件函数体内对 props 的
  直接读取仍是一次性（`$state(props.x)` initial-only，与主流框架一致）；prop 信号是真实信号，进入
  依赖图/journal/HMR 按序还原；契约校验经免追踪包裹，prop 变化不会无谓触发分支重选。
...
- **静态依赖图第一期（2026-08-30，F-2）**：codegen 发射器在每个响应式挂点（bindExpr / {#if} test /
  {#each} expr+key+子树）编译期提取表达式根标识符（`exprRootIdents`，expr.ts 与求值器同 tokenizer），
  产物携带 `deps: { reactive, mount, events }` 清单（组件 props=挂载期桶、on:=事件期桶）。
  **语义边界（诚实标注）**：清单是**语法级引用集**——含 {#if}/{:else} 与 &&/||/?: 未执行分支、
  滤除 {#each} 子作用域变量——是运行时追踪集的**超集**；等号仅在"无短路 + 全部经 .value 读信号"
...
- **静态依赖图第二期·构建期图查询（2026-09-06，F-2）**：清单从惰性元数据转正为查询工件——
  ...
  改代码前影响面速查（无需运行应用），活图=运行时真实依赖。
- **静态依赖图第二期·跳过追踪快路径（2026-09-06，F-2）**：`$effectStatic(fn, deps)` 进内核——
  依赖创建时精确给出、运行期零追踪簿记，fn 全程无追踪上下文求值（首跑嵌套在分支重建的追踪期时
  置 null，不污染外层依赖）。接入点 = `bindExpr` 分流（解释器/codegen 同走此函数 ⇒ 双路径同源），
  exactness 判据（保守，不确定即回退动态追踪）：①无函数调用（模板层 ATR-301 本就拒绝括号，
  守卫为未来语法扩展的纵深防御）；②全部根标识符在 scope 解析为信号（根非信号——如 F-5 props
  getter——其属性链深处可能藏信号读，静态订阅会漏）；③至少一个依赖。短路/三元不拒：静态集 ⊇
  实读集只会良性超订阅（多触发一次重算，输出不变）。验证：5 专项用例（白盒 + 判据边界）+ 122 全绿
  + 六正控 6/6 + bench 四指标全 PASS（9.09KB/3.2ms/52ms/267ms）。**剩余二期**：prod 剥离。
