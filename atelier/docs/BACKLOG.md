...
> | **P0-8（新）评分器与 footgun 修复（2026-08-30，M3 收官后）** | ✅ 三项全落地：① `grade.mjs` 硬禁检查改打剥注释后的代码文本（`harness/strip-comments.ts` 零依赖状态机，正则字面量/模板串 `${}` 嵌套保守处理——宁可漏剥报 FAIL 不误剥放违规；自测用例入 harness + 双向 fixture `harness/fixtures/`：注释提及必须 PASS、真用定时器必须 FAIL；三正控复跑 3/3）；② store 快照按引用 footgun：runtime JSDoc 定性 + 应用模板新守卫 `tests/state-discipline.test.ts`（静态拦 .value 的 push/属性赋值/下标赋值三类原地修改，双向实证）+ state-transactions 技能包 Rules/Common-failures 双补；③ expr 表达式错误前置化：`=>` 箭头 / `=` 赋值 / 调用遗留 token 三路 ATR-301 带修法报错（原先 `=` 只报"无法解析字符"、调用被**静默丢尾求出函数本身**——现均显式拒绝），error-codes 技能包 ATR-301 行同步；expr.ts 全文占位 ATR-3xx 统一为注册码 ATR-301。43/43 测试绿 + check-skills 31/31。**诚实边界：已存在应用是 init 时点 vendor 拷贝，要拿新守卫测试须重新同步 tests/**；结构性深拷贝快照（替代按引用语义）因任意值类型拷贝不可靠暂不采用（决策 5 注记）。**路线决策（2026-08-30，用户定论）：技能包是核心竞争力、保留；先壮大框架功能（丰富能力层），与 React 的对照对决后置——干扰面实验与加难任务层（M3 报告 §4）挂起待功能扩张后再排** |
