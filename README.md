...
| 渲染性能 | 10³ 节点挂载+首渲染 ≤ 50 ms | **3.4 ms**（1254 节点，7 样本中位） | PASS |
| 开发反馈（HMR） | 保存→可见 ≤ 100 ms | **51 ms**（.atr.ts accept 路径：保值热交换重挂载，$state 不清零） | PASS |
| 截图回环 | dev 单组件截图 diff ≤ 500 ms | **1933 ms**（每拍拉起瞬态无头实例） | FAIL → P0-6 工单 |
