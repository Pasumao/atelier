...
import "./atelier-tailwind.css"; // 决策 16：token 派生 + Tailwind AOT 编译产物（生成产物）
import "./atelier-ui.css"; // 决策 16：recipe 层种子（.btn/.ppanel/.tab + 全局 keyframes）
import { initTokens, mountComponent, registry, validateFlat, devFetch } from "./runtime";
...
import { installStateBridge, store } from "./runtime";
installStateBridge();

// 会话初始锚点：代理经 MCP `checkpoint.list` 即有非空时间线可依
store.commit("session-start");

// 控制台自检：注册表查询面（决策 7 雏形；devFetch 携 token，避免 ATR-402 噪音）
